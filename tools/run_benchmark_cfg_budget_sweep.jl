...
const DEFAULT_SWEEP_OUTPUT_DIR = joinpath(pwd(), "tmp", "benchmark_cfg_budget_sweep")
const SWEEP_COMPLEXITY = 5
const SWEEP_NUM_SENTENCES = 4_000
const SWEEP_EPOCHS = [1, 2, 4]

function main(args)
    length(args) <= 1 || error("usage: julia --project=tools/benchmark_cfg tools/run_benchmark_cfg_budget_sweep.jl [output_dir]")
    ...
    for epochs in SWEEP_EPOCHS
        run_name = "cfg_complexity_$(SWEEP_COMPLEXITY)_epochs_$(epochs)"
        run_output_dir = joinpath(sweep_output_dir, run_name)
        settings = merge_settings(base_settings; epochs = epochs)
        metrics = run_experiment(
            settings,
            run_output_dir;
            experiment_name = run_name,
            purpose = "fixed-complexity synthetic CFG budget sweep for pipeline evaluation",
        )
        push!(run_summaries, summarize_run(metrics))
    end

    summary = Dict(
        "experiment" => "benchmark_cfg_budget_sweep",
        "purpose" => "controlled epoch-only sweep at fixed CFG complexity for the existing KeemenaLM experiment pipeline",
        "sweep_variable" => "epochs",
        "held_fixed" => Dict(
            "complexity" => SWEEP_COMPLEXITY,
            "num_sentences" => SWEEP_NUM_SENTENCES,
...
function write_summary_markdown(path::AbstractString, summary::Dict{String, Any})
    open(path, "w") do io
        println(io, "# BenchmarkDataNLP CFG budget sweep")
        println(io)
        ...
        println(
            ...
            "`, same model, same Flux training path, same checkpoint/bundle flow",
        )
        println(io)
        println(io, "| Epochs | Train Loss | Validation Loss | Test Loss | Test PPL | Final Step | Train Tokens | Sample Outputs |")
        println(io, "| --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |")
