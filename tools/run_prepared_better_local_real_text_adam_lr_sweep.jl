...
const ADAM_LR_SWEEP_OUTPUT_DIR = joinpath(pwd(), "tmp", "prepared_better_local_real_text_adam_lr_sweep")
const ADAM_LR_SWEEP_SUMMARY_JSON = "summary.json"
const ADAM_LR_SWEEP_SUMMARY_MD = "summary.md"

const ADAM_LR_SWEEP_RUNS = [
    (label = "adam_lr_0p0010", learning_rate = 0.001f0),
    (label = "adam_lr_0p0005", learning_rate = 0.0005f0),
    (label = "adam_lr_0p0002", learning_rate = 0.0002f0),
]

function main(args)
    length(args) <= 2 || error("usage: julia --project=tools/benchmark_cfg tools/run_prepared_better_local_real_text_adam_lr_sweep.jl [prepared_dataset_dir] [output_dir]")
    dataset_dir = length(args) >= 1 ? abspath(args[1]) : PREPARED_CORPUS_INPUT_DIR
    output_dir = length(args) == 2 ? abspath(args[2]) : ADAM_LR_SWEEP_OUTPUT_DIR
    run_prepared_better_local_real_text_adam_lr_sweep(dataset_dir, output_dir)
end

function run_prepared_better_local_real_text_adam_lr_sweep(dataset_dir::AbstractString, output_dir::AbstractString)
    mkpath(output_dir)
    ...
    isfile(baseline_metrics_path) || throw(ArgumentError("Adam baseline metrics not found: $(baseline_metrics_path)"))
    baseline_metrics = JSON3.read(read(baseline_metrics_path, String))

    run_summaries = Dict{String, Any}[]
    for run in ADAM_LR_SWEEP_RUNS
        ...
        metrics = if isfile(run_metrics_path)
            ...
            run_prepared_better_local_real_text_experiment(
                ...
                run_output_dir;
                settings = settings,
                experiment_name = "prepared_better_local_real_text_adam_lr_sweep_" * run.label,
                purpose = "Adam learning-rate sweep on the prepared better local real-text corpus at the current best recipe, not chatbot benchmarking",
                optimizer_builder = experiment_settings -> Flux.Adam(experiment_settings.learning_rate),
                optimizer_name = "Flux.Adam",
                optimizer_hparams = Dict("learning_rate" => run.learning_rate),
            )
        end
        ...
        push!(
            ...
            Dict(
                ...
                "optimizer_name" => "Flux.Adam",
                "optimizer_hyperparameters" => Dict("learning_rate" => run.learning_rate),
    ...
    summary = Dict(
        "experiment" => "prepared_better_local_real_text_adam_lr_sweep",
        "purpose" => "Adam learning-rate sweep on the prepared better local real-text corpus at the current best recipe",
    ...
    open(joinpath(output_dir, ADAM_LR_SWEEP_SUMMARY_JSON), "w") do io
        JSON3.write(io, summary)
    end
    write_adam_lr_sweep_summary_md(joinpath(output_dir, ADAM_LR_SWEEP_SUMMARY_MD), summary)

    println("adam lr sweep summary: ", joinpath(output_dir, ADAM_LR_SWEEP_SUMMARY_JSON))
    println("adam lr sweep markdown: ", joinpath(output_dir, ADAM_LR_SWEEP_SUMMARY_MD))
    return summary
end

function write_adam_lr_sweep_summary_md(path::AbstractString, summary)
    baseline = summary["baseline"]
    runs = summary["runs"]
    open(path, "w") do io
        println(io, "# Prepared Better Local Real-Text Adam LR Sweep")
        println(io)
        ...
        println(io)
        println(io, "| run | learning_rate | train_loss | val_loss | test_loss | train_examples | train_batches | final_step |")
        println(io, "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |")
        for run in runs
            println(
                io,
                "| $(run["run_name"]) | $(run["learning_rate"]) | $(round(run["train_loss"], digits=4)) | " *
