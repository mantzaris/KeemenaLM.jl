...
const TINY_CHATBOT_SUBWORD_OUTPUT_DIR = joinpath(pwd(), "tmp", "tiny_chatbot_demo_subword_run_v1")
const TINY_CHATBOT_SUBWORD_EXPERIMENT_NAME = "tiny_chatbot_demo_subword_run_v1"
const TINY_CHATBOT_SUBWORD_PURPOSE = "first proper subword conversational chatbot run on the tiny conversational demo dataset"
...
Base.@kwdef struct TinyChatbotSubwordSettings
    ...
    tokenizer_model_name::String = "tiny_chatbot_demo_gpt2_bytebpe"
...
function main(args)
    ...
    while argument_index <= length(args)
        ...
        elseif argument == "--epochs"
            ...
            error("usage: julia --project=tools/subword_real_text tools/run_tiny_chatbot_demo_subword_v1.jl [--dataset-dir DIR] [--output-dir DIR] [--epochs N]")
        end
        argument_index += 1
    end
...
function run_tiny_chatbot_subword_demo(
    ...
    trainer = KeemenaLM.Core.Trainer(
        ...
        metadata = Dict(
            ...
            "corpus_source" => "tiny_conversational_chatbot_demo_dataset",
    ...
    println("== Tiny conversational chatbot subword run ==")
