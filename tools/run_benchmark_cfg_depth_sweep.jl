...
const DEFAULT_SWEEP_OUTPUT_DIR = joinpath(pwd(), "tmp", "benchmark_cfg_depth_sweep")
...
const SWEEP_CONTEXT_LENGTH = 48
const SWEEP_NUM_HEADS = 2
const SWEEP_EMBEDDING_SIZE = 64
const SWEEP_FFN_HIDDEN_SIZE = 128
const DEPTHS = [2, 3, 4]

function main(args)
    length(args) <= 1 || error("usage: julia --project=tools/benchmark_cfg tools/run_benchmark_cfg_depth_sweep.jl [output_dir]")
    ...
    base_settings = merge_settings(
        ...
        context_length = SWEEP_CONTEXT_LENGTH,
        num_heads = SWEEP_NUM_HEADS,
        embedding_size = SWEEP_EMBEDDING_SIZE,
        ffn_hidden_size = SWEEP_FFN_HIDDEN_SIZE,
    )

    run_summaries = Dict{String, Any}[]
    for num_layers in DEPTHS
        run_name = "cfg_complexity_$(SWEEP_COMPLEXITY)_layers_$(num_layers)"
        run_output_dir = joinpath(sweep_output_dir, run_name)
        settings = merge_settings(base_settings; num_layers = num_layers)
        metrics = run_experiment(
            settings,
            run_output_dir;
            experiment_name = run_name,
            purpose = "fixed-complexity synthetic CFG depth sweep for pipeline evaluation",
        )
        push!(run_summaries, summarize_run(metrics))
    end

    summary = Dict(
        "experiment" => "benchmark_cfg_depth_sweep",
        "purpose" => "controlled depth-only sweep at fixed CFG complexity for the existing KeemenaLM experiment pipeline",
        "sweep_variable" => "num_layers",
        "held_fixed" => Dict(
            "complexity" => SWEEP_COMPLEXITY,
            "num_sentences" => SWEEP_NUM_SENTENCES,
            "epochs" => SWEEP_EPOCHS,
...
function summarize_run(metrics)::Dict{String, Any}
    ...
    return Dict(
        ...
        "num_layers" => metrics["model"]["num_layers"],
...
function write_summary_markdown(path::AbstractString, summary::Dict{String, Any})
    open(path, "w") do io
        println(io, "# BenchmarkDataNLP CFG depth sweep")
        println(io)
        ...
        println(
            ...
            "`, `embedding_size = ",
            summary["held_fixed"]["embedding_size"],
            "`, same Flux training path, same checkpoint/bundle flow",
        )
        println(io)
        println(io, "| Layers | Train Loss | Validation Loss | Test Loss | Test PPL | Final Step | Train Tokens | Sample Outputs |")
        println(io, "| --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |")
