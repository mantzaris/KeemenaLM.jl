...
const ADAM_BUDGET_SWEEP_STAGE2_OUTPUT_DIR = joinpath(pwd(), "tmp", "prepared_better_local_real_text_adam_budget_sweep_stage2")
const ADAM_BUDGET_SWEEP_STAGE2_SUMMARY_JSON = "summary.json"
const ADAM_BUDGET_SWEEP_STAGE2_SUMMARY_MD = "summary.md"

const ADAM_BUDGET_SWEEP_STAGE2_RUNS = [
    (epochs = 30,),
    (epochs = 34,),
    (epochs = 38,),
]
...
function main(args)
    length(args) <= 2 || error("usage: julia --project=tools/benchmark_cfg tools/run_prepared_better_local_real_text_adam_budget_sweep_stage2.jl [prepared_dataset_dir] [output_dir]")
    dataset_dir = length(args) >= 1 ? abspath(args[1]) : PREPARED_CORPUS_INPUT_DIR
    output_dir = length(args) == 2 ? abspath(args[2]) : ADAM_BUDGET_SWEEP_STAGE2_OUTPUT_DIR
    run_prepared_better_local_real_text_adam_budget_sweep_stage2(dataset_dir, output_dir)
end

function run_prepared_better_local_real_text_adam_budget_sweep_stage2(dataset_dir::AbstractString, output_dir::AbstractString)
    mkpath(output_dir)

    baseline_metrics_path = joinpath(
        pwd(),
        "tmp",
        "prepared_better_local_real_text_adam_budget_sweep",
        "epochs_30",
        "metrics.json",
    )
    isfile(baseline_metrics_path) || throw(ArgumentError("Adam 30-epoch baseline metrics not found: $(baseline_metrics_path)"))
    baseline_metrics = JSON3.read(read(baseline_metrics_path, String))

    run_summaries = Dict{String, Any}[]
    for run in ADAM_BUDGET_SWEEP_STAGE2_RUNS
        ...
        metrics = if isfile(run_metrics_path)
            ...
            run_prepared_better_local_real_text_experiment(
                ...
                run_output_dir;
                settings = settings,
                experiment_name = "prepared_better_local_real_text_adam_budget_sweep_stage2_" * run_name,
                purpose = "further Adam budget extension sweep on the prepared better local real-text corpus at the current best recipe, not chatbot benchmarking",
    ...
    summary = Dict(
        "experiment" => "prepared_better_local_real_text_adam_budget_sweep_stage2",
        "purpose" => "further Adam budget-extension sweep on the prepared better local real-text corpus at the current best recipe",
    ...
    open(joinpath(output_dir, ADAM_BUDGET_SWEEP_STAGE2_SUMMARY_JSON), "w") do io
        JSON3.write(io, summary)
    end
    write_adam_budget_sweep_stage2_summary_md(joinpath(output_dir, ADAM_BUDGET_SWEEP_STAGE2_SUMMARY_MD), summary)

    println("adam budget sweep summary: ", joinpath(output_dir, ADAM_BUDGET_SWEEP_STAGE2_SUMMARY_JSON))
    println("adam budget sweep markdown: ", joinpath(output_dir, ADAM_BUDGET_SWEEP_STAGE2_SUMMARY_MD))
    return summary
end

function write_adam_budget_sweep_stage2_summary_md(path::AbstractString, summary)
    baseline = summary["baseline"]
    runs = summary["runs"]
    open(path, "w") do io
        println(io, "# Prepared Better Local Real-Text Adam Budget Sweep Stage 2")
        println(io)
