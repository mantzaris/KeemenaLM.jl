...
const BUDGET_SWEEP_STAGE3_OUTPUT_DIR = joinpath(pwd(), "tmp", "prepared_better_local_real_text_budget_sweep_stage3")
const BUDGET_SWEEP_STAGE3_SUMMARY_JSON = "summary.json"
const BUDGET_SWEEP_STAGE3_SUMMARY_MD = "summary.md"

const BUDGET_SWEEP_STAGE3_RUNS = [
    (epochs = 10,),
    (epochs = 12,),
    (epochs = 14,),
]
...
function main(args)
    length(args) <= 2 || error("usage: julia --project=tools/benchmark_cfg tools/run_prepared_better_local_real_text_budget_sweep_stage3.jl [prepared_dataset_dir] [output_dir]")
    dataset_dir = length(args) >= 1 ? abspath(args[1]) : PREPARED_CORPUS_INPUT_DIR
    output_dir = length(args) == 2 ? abspath(args[2]) : BUDGET_SWEEP_STAGE3_OUTPUT_DIR
    run_prepared_better_local_real_text_budget_sweep_stage3(dataset_dir, output_dir)
end

function run_prepared_better_local_real_text_budget_sweep_stage3(dataset_dir::AbstractString, output_dir::AbstractString)
    mkpath(output_dir)

    baseline_metrics_path = joinpath(
        pwd(),
        "tmp",
        "prepared_better_local_real_text_budget_sweep_stage2",
        "epochs_10",
        "metrics.json",
    )
    isfile(baseline_metrics_path) || throw(ArgumentError("128/256, 10-epoch baseline metrics not found: $(baseline_metrics_path)"))
    baseline_metrics = JSON3.read(read(baseline_metrics_path, String))

    run_summaries = Dict{String, Any}[]
    for run in BUDGET_SWEEP_STAGE3_RUNS
        ...
        metrics = run_prepared_better_local_real_text_experiment(
            ...
            run_output_dir;
            settings = settings,
            experiment_name = "prepared_better_local_real_text_budget_stage3_" * run_name,
            purpose = "extended prepared better local real-text budget sweep at the current best recipe, not chatbot benchmarking",
        )
    ...
    summary = Dict(
        "experiment" => "prepared_better_local_real_text_budget_sweep_stage3",
        "purpose" => "extended training-budget sweep on the prepared better local real-text corpus at the current best recipe",
    ...
    open(joinpath(output_dir, BUDGET_SWEEP_STAGE3_SUMMARY_JSON), "w") do io
        JSON3.write(io, summary)
    end
    write_budget_sweep_stage3_summary_md(joinpath(output_dir, BUDGET_SWEEP_STAGE3_SUMMARY_MD), summary)

    println("budget extension summary: ", joinpath(output_dir, BUDGET_SWEEP_STAGE3_SUMMARY_JSON))
    println("budget extension markdown: ", joinpath(output_dir, BUDGET_SWEEP_STAGE3_SUMMARY_MD))
    return summary
end

function write_budget_sweep_stage3_summary_md(path::AbstractString, summary)
    baseline = summary["baseline"]
    runs = summary["runs"]
    open(path, "w") do io
        println(io, "# Prepared Better Local Real-Text Budget Sweep Stage 3")
        println(io)
