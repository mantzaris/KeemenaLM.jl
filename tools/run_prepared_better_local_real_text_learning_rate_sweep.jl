...
const LR_SWEEP_OUTPUT_DIR = joinpath(pwd(), "tmp", "prepared_better_local_real_text_learning_rate_sweep")
const LR_SWEEP_SUMMARY_JSON = "summary.json"
const LR_SWEEP_SUMMARY_MD = "summary.md"

const LR_SWEEP_RUNS = [
    (label = "lr_0p010", learning_rate = 0.01f0),
    (label = "lr_0p005", learning_rate = 0.005f0),
    (label = "lr_0p002", learning_rate = 0.002f0),
]

function main(args)
    length(args) <= 2 || error("usage: julia --project=tools/benchmark_cfg tools/run_prepared_better_local_real_text_learning_rate_sweep.jl [prepared_dataset_dir] [output_dir]")
    dataset_dir = length(args) >= 1 ? abspath(args[1]) : PREPARED_CORPUS_INPUT_DIR
    output_dir = length(args) == 2 ? abspath(args[2]) : LR_SWEEP_OUTPUT_DIR
    run_prepared_better_local_real_text_learning_rate_sweep(dataset_dir, output_dir)
end

function run_prepared_better_local_real_text_learning_rate_sweep(dataset_dir::AbstractString, output_dir::AbstractString)
    mkpath(output_dir)
    ...
    for run in LR_SWEEP_RUNS
        ...
        metrics = run_prepared_better_local_real_text_experiment(
            ...
            run_output_dir;
            settings = settings,
            experiment_name = "prepared_better_local_real_text_lr_sweep_" * run.label,
            purpose = "first learning-rate sweep on the prepared better local real-text corpus at the current best recipe, not chatbot benchmarking",
        )
    ...
    summary = Dict(
        "experiment" => "prepared_better_local_real_text_learning_rate_sweep",
        "purpose" => "learning-rate sweep on the prepared better local real-text corpus at the current best recipe",
    ...
    open(joinpath(output_dir, LR_SWEEP_SUMMARY_JSON), "w") do io
        JSON3.write(io, summary)
    end
    write_learning_rate_sweep_summary_md(joinpath(output_dir, LR_SWEEP_SUMMARY_MD), summary)

    println("learning-rate sweep summary: ", joinpath(output_dir, LR_SWEEP_SUMMARY_JSON))
    println("learning-rate sweep markdown: ", joinpath(output_dir, LR_SWEEP_SUMMARY_MD))
    return summary
end

function write_learning_rate_sweep_summary_md(path::AbstractString, summary)
    baseline = summary["baseline"]
    runs = summary["runs"]
    open(path, "w") do io
        println(io, "# Prepared Better Local Real-Text Learning-Rate Sweep")
        println(io)
        println(io, "Baseline: epochs=$(baseline["epochs"]), lr=$(baseline["learning_rate"]), embed=$(baseline["embedding_size"]), ffn=$(baseline["ffn_hidden_size"]), context=$(baseline["context_length"]), test_loss=$(round(baseline["test_loss"], digits=4))")
        println(io)
