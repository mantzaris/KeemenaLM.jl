...
const CHATBOT_SUBWORD_OUTPUT_DIR = joinpath(pwd(), "tmp", "keemena_docs_assistant_chatbot_subword_poc_run")
const CHATBOT_SUBWORD_EXPERIMENT_NAME = "keemena_docs_assistant_chatbot_subword_poc_run"
const CHATBOT_SUBWORD_PURPOSE = "single subword-tokenizer chatbot comparison for the Keemena Docs Assistant PoC"
const CHATBOT_DOCUMENT_SEPARATOR = "\n<CHAT_END>\n"

Base.@kwdef struct ChatbotSubwordSettings
    ...
    sample_generation_tokens::Int = 96
    tokenizer_trainer::Symbol = :hf_gpt2_bytebpe
    tokenizer_vocab_size::Int = 384
    tokenizer_min_frequency::Int = 2
    tokenizer_model_name::String = "keemena_docs_assistant_gpt2_bytebpe"
    document_separator::String = CHATBOT_DOCUMENT_SEPARATOR
end
...
function main(args)
    ...
    output_dir = CHATBOT_SUBWORD_OUTPUT_DIR
    settings = ChatbotSubwordSettings()
    ...
    while argument_index <= length(args)
        ...
        elseif argument == "--epochs"
            ...
            error("usage: julia --project=tools/subword_real_text tools/run_keemena_docs_assistant_chatbot_subword_poc.jl [--dataset-dir DIR] [--output-dir DIR] [--epochs N]")
        end
        argument_index += 1
    end

    run_chatbot_subword_poc(dataset_dir, output_dir; settings = settings)
end

function run_chatbot_subword_poc(
    dataset_dir::AbstractString,
    output_dir::AbstractString;
    settings::ChatbotSubwordSettings = ChatbotSubwordSettings(),
)
    ...
    tokenizer_training_output = KeemenaSubwords.quick_train_bundle(
        ...
        sanity_text = "User: How do I load a saved bundle?\nAssistant:",
    )
    ...
    trainer = KeemenaLM.Core.Trainer(
        ...
        backend = :flux,
        metadata = Dict(
            "experiment" => CHATBOT_SUBWORD_EXPERIMENT_NAME,
            "corpus_source" => "prepared_local_chatbot_docs_dataset",
    ...
    println("== Keemena Docs Assistant chatbot subword PoC ==")
    ...
    for epoch in 1:settings.epochs
        ...
        save_checkpoint(checkpoint_path, trainer, model; experiment = CHATBOT_SUBWORD_EXPERIMENT_NAME, epoch = epoch)
    ...
    save_checkpoint(final_checkpoint_path, trainer, model; experiment = CHATBOT_SUBWORD_EXPERIMENT_NAME, stage = "final")
    ...
    metrics = Dict(
        "experiment" => CHATBOT_SUBWORD_EXPERIMENT_NAME,
        "purpose" => CHATBOT_SUBWORD_PURPOSE,
        "comparison_note" => "Loss is not directly apples-to-apples with the char-level chatbot baseline because tokenization changed. Qualitative outputs and overall degeneration are the more useful comparison.",
        "corpus" => Dict(
...
function subword_recipe_dict(dataset_dir::AbstractString, output_dir::AbstractString, settings::ChatbotSubwordSettings)
    return Dict(
        "experiment" => CHATBOT_SUBWORD_EXPERIMENT_NAME,
        "purpose" => CHATBOT_SUBWORD_PURPOSE,
