...
const DEFAULT_OUTPUT_DIRECTORY = joinpath(@__DIR__, "..", "tmp", "tiny_chatbot_demo_dataset_v3")
...
const TARGET_EXTERNAL_SINGLE_TURN_MAX = 320
const TARGET_EXTERNAL_MULTI_TURN_MAX = 120
...
function valid_external_record(record::Dict{String,Any})::Bool
    ...
    rank = record["rank"]
    (rank === missing || rank == 0) || return false
    return true
end
...
function external_pair_usable(prompt::String, answer::String)::Bool
    external_conversation_start_usable(prompt, answer) || return false
    ordinary_assistant_prompt(prompt) || return false
    return true
end
...
function external_conversation_start_usable(prompt::String, answer::String)::Bool
    ...
    3 <= prompt_words <= 24 || return false
    4 <= answer_words <= 60 || return false
    12 <= prompt_chars <= 240 || return false
    20 <= answer_chars <= 320 || return false

    occursin('\n', prompt) && return false
    count(==('\n'), answer) > 1 && return false
    count(==(':'), answer) > 3 && return false
    ...
    reject_keywords = [
        "roleplay", "pretend you are", "write a story", "short story", "fanfiction", "poem",
        ...
        "fictional kingdom", "dragon", "wizard", "vampire",
        ...
        "mermaid", "wabbit", "duck season", "landlord", "tenant",
        "github", "git remote", "css3", "browser", "priority queue",
        "java", "c#", "c ++", "lego", "diablo", "elden ring",
        "windows defender", "mcaffe", "mcafee", "equilateral triangle",
...
function external_follow_up_usable(prompt::String, answer::String)::Bool
    ...
    2 <= prompt_words <= 20 || return false
    4 <= answer_words <= 55 || return false
    8 <= prompt_chars <= 180 || return false
    16 <= answer_chars <= 280 || return false
    occursin('\n', prompt) && return false
    count(==('\n'), answer) > 1 && return false
    count(==(':'), answer) > 3 && return false
    occursin("http://", prompt) && return false
    ...
    follow_up_rejects = [
        ...
        "github", "java", "css3", "browser", "priority queue",
        "windows defender", "mcafee", "equilateral triangle",
        "limerick", "rhyme with", "fever", "temperature",
        "open assistant", "general knowledge", "the force", "skywalker", "jedi",
    ]
...
function ordinary_assistant_prompt(prompt::String)::Bool
    lowered = lowercase(strip(prompt))
    startswith_patterns = [
        "can you", "could you", "would you", "help me", "how do i", "how can i",
        "what should i", "please", "i need", "i'm ", "im ", "i am ",
        "rewrite", "rephrase", "explain", "summarize", "suggest", "give me",
        "plan ", "clarify", "improve", "make this", "tell me how",
        "what's a good way", "what is a ", "what is an ", "what does ",
        "is it okay", "should i", "thank you", "thanks", "hi", "hello", "good morning", "good evening",
    ]
    any(pattern -> startswith(lowered, pattern), startswith_patterns) && return true
    occursin(r"\bhelp me\b", lowered) && return true
    occursin(r"\bcan you help\b", lowered) && return true
    return false
end
...
function build_metadata(examples, training, validation, testing, local_metadata, external_status)
    ...
    return Dict(
        "dataset_name" => "tiny_chatbot_demo_dataset_v3",
        "intended_scope" => "Filtered small-model conversational chatbot corpus for Julia. Small, polite, plainspoken assistant for short back-and-forth chat, simple explanations, light planning, clarification, harmless requests, and a small amount of Keemena-flavored technical help as secondary flavor.",
        ...
        "external_filtering_rules" => [
            "English only",
            "review_result=true, not deleted, not synthetic, and prefer rank 0 or missing rank",
            "extract both single-turn and short multi-turn assistant dialogues from OASST trees",
            "first user turn must look like an ordinary assistant/help request rather than trivia, code-helpdesk, or theatrical chat",
            "follow-up user turn uses a softer conversational filter so short natural back-and-forth exchanges survive",
            "short to medium prompt and answer lengths with low formatting noise",
            "reject roleplay, fiction, erotic/sexual, self-harm, violence, illegal, malware, hateful, bizarre, policy-heavy, citation-heavy, and code-heavy shapes",
            "reject trivia-heavy, joke-heavy, code-helpdesk, game fandom, browser/version lookup, and other oddball shapes surfaced during inspection",
            "reject URL-heavy, overly long, or messy examples",
            "keep the external slice filtered, inspectable, and conversation-first rather than broad",
        ],
        "dataset_counts" => Dict(
