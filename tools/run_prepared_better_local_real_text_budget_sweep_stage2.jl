...
const BUDGET_SWEEP_STAGE2_OUTPUT_DIR = joinpath(pwd(), "tmp", "prepared_better_local_real_text_budget_sweep_stage2")
const BUDGET_SWEEP_STAGE2_SUMMARY_JSON = "summary.json"
const BUDGET_SWEEP_STAGE2_SUMMARY_MD = "summary.md"

const BUDGET_SWEEP_STAGE2_RUNS = [
    (epochs = 6,),
    (epochs = 8,),
    (epochs = 10,),
]
...
function main(args)
    length(args) <= 2 || error("usage: julia --project=tools/benchmark_cfg tools/run_prepared_better_local_real_text_budget_sweep_stage2.jl [prepared_dataset_dir] [output_dir]")
    dataset_dir = length(args) >= 1 ? abspath(args[1]) : PREPARED_CORPUS_INPUT_DIR
    output_dir = length(args) == 2 ? abspath(args[2]) : BUDGET_SWEEP_STAGE2_OUTPUT_DIR
    run_prepared_better_local_real_text_budget_sweep_stage2(dataset_dir, output_dir)
end

function run_prepared_better_local_real_text_budget_sweep_stage2(dataset_dir::AbstractString, output_dir::AbstractString)
    mkpath(output_dir)
    ...
    for run in BUDGET_SWEEP_STAGE2_RUNS
        ...
        metrics = run_prepared_better_local_real_text_experiment(
            ...
            run_output_dir;
            settings = settings,
            experiment_name = "prepared_better_local_real_text_budget_stage2_" * run_name,
            purpose = "follow-up prepared better local real-text budget sweep at the current best recipe, not chatbot benchmarking",
        )
    ...
    summary = Dict(
        "experiment" => "prepared_better_local_real_text_budget_sweep_stage2",
        "purpose" => "follow-up training-budget sweep on the prepared better local real-text corpus at the current best recipe",
    ...
    open(joinpath(output_dir, BUDGET_SWEEP_STAGE2_SUMMARY_JSON), "w") do io
        JSON3.write(io, summary)
    end
    write_budget_sweep_stage2_summary_md(joinpath(output_dir, BUDGET_SWEEP_STAGE2_SUMMARY_MD), summary)

    println("budget follow-up summary: ", joinpath(output_dir, BUDGET_SWEEP_STAGE2_SUMMARY_JSON))
    println("budget follow-up markdown: ", joinpath(output_dir, BUDGET_SWEEP_STAGE2_SUMMARY_MD))
    return summary
end

function write_budget_sweep_stage2_summary_md(path::AbstractString, summary)
    baseline = summary["baseline"]
    runs = summary["runs"]
    open(path, "w") do io
        println(io, "# Prepared Better Local Real-Text Budget Sweep Stage 2")
        println(io)
