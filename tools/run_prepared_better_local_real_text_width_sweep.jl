...
const WIDTH_SWEEP_OUTPUT_DIR = joinpath(pwd(), "tmp", "prepared_better_local_real_text_width_sweep")
const WIDTH_SWEEP_SUMMARY_JSON = "summary.json"
const WIDTH_SWEEP_SUMMARY_MD = "summary.md"

const WIDTH_SWEEP_RUNS = [
...
function main(args)
    length(args) <= 2 || error("usage: julia --project=tools/benchmark_cfg tools/run_prepared_better_local_real_text_width_sweep.jl [prepared_dataset_dir] [output_dir]")
    dataset_dir = length(args) >= 1 ? abspath(args[1]) : PREPARED_CORPUS_INPUT_DIR
    output_dir = length(args) == 2 ? abspath(args[2]) : WIDTH_SWEEP_OUTPUT_DIR
    run_prepared_better_local_real_text_width_sweep(dataset_dir, output_dir)
end

function run_prepared_better_local_real_text_width_sweep(dataset_dir::AbstractString, output_dir::AbstractString)
    mkpath(output_dir)

    baseline_metrics_path = joinpath(pwd(), "tmp", "prepared_better_local_real_text_experiment", "metrics.json")
    isfile(baseline_metrics_path) || throw(ArgumentError("char prepared-corpus baseline metrics not found: $(baseline_metrics_path)"))
    baseline_metrics = JSON3.read(read(baseline_metrics_path, String))

    run_summaries = Dict{String, Any}[]
    for run in WIDTH_SWEEP_RUNS
        ...
        metrics = run_prepared_better_local_real_text_experiment(
            ...
            run_output_dir;
            settings = settings,
            experiment_name = "prepared_better_local_real_text_width_" * run_name,
            purpose = "prepared better local real-text width sweep, not chatbot benchmarking",
        )
    ...
    summary = Dict(
        "experiment" => "prepared_better_local_real_text_width_sweep",
        "purpose" => "controlled model-width sweep on the prepared better local real-text corpus using the char-level path",
    ...
    open(joinpath(output_dir, WIDTH_SWEEP_SUMMARY_JSON), "w") do io
        JSON3.write(io, summary)
    end
    write_width_sweep_summary_md(joinpath(output_dir, WIDTH_SWEEP_SUMMARY_MD), summary)

    println("width sweep summary: ", joinpath(output_dir, WIDTH_SWEEP_SUMMARY_JSON))
    println("width sweep markdown: ", joinpath(output_dir, WIDTH_SWEEP_SUMMARY_MD))
    return summary
end

function write_width_sweep_summary_md(path::AbstractString, summary)
    baseline = summary["baseline"]
    runs = summary["runs"]
    open(path, "w") do io
        println(io, "# Prepared Better Local Real-Text Width Sweep")
        println(io)
        println(io, "Baseline: embed=$(baseline["embedding_size"]), ffn=$(baseline["ffn_hidden_size"]), test_loss=$(round(baseline["test_loss"], digits=4))")
        println(io)
